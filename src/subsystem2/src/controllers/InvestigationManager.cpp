...
// FIX: was  #include "ipc_manager.h"  (bare filename — breaks when compiled
//      from any directory other than the ipc/include folder itself).
//      Corrected to the project-relative path used by all other managers.
...
namespace subsystem2
{
                                                                   ...
                                                                   ChargeSheet *&out_sheet)
    {
        ...
        std::string sql = "INSERT INTO public.charge_sheets (case_id, filed_by) VALUES (" + std::to_string(case_id) + ", " + std::to_string(session.officerId) + ") RETURNING charge_sheet_id, charge_sheet_number;";
                                                                    ...
                                                                    const JusticeFlow::SessionContext &session)
    {
        ...
        // 1. Execute Domain Logic FIRST (check laws invoked, check not already locked)
        ...
        if (domain_res != JusticeFlow::ResultCode::OK)
        {
            ...
            return domain_res; // RECORD_LOCKED or INVALID_STATE
        }
        ...
        std::string sql = "UPDATE public.charge_sheets SET "
                          ...
                          "submitted_by = " +
                          std::to_string(session.officerId) + ", "
                                                              "locked_at = now(), "
                                                              "locked_by = " +
                          std::to_string(session.officerId) + " "
                                                              "WHERE charge_sheet_id = " +
                          std::to_string(sheet->getSheetId()) + ";";
